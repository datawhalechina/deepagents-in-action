...
PostgresStore 提供真正的持久化——数据写入 PostgreSQL 数据库，即使应用重启也不会丢失。
...
通过 `StoreBackend` 存储的文件使用以下 JSON 格式：

```python
{
    "content": ["第一行", "第二行", "第三行"],  # 按行存储的字符串列表
    "created_at": "2024-01-15T10:30:00Z",       # 创建时间
    "modified_at": "2024-01-15T11:45:00Z"       # 最后修改时间
}
```

在 Agent 外部（后端服务、初始化脚本）预填记忆，使用 `create_file_data` 辅助函数：
